...
thermiqueRoutes.post('/measurement/create', async (c) => {
  try {
    ...
    // Validation
    if (!audit_token && !intervention_id) { // audit_token OR intervention_id required
      return c.json({ 
        success: false, 
        error: 'audit_token ou intervention_id requis' 
      }, 400);
    }
...
thermiqueRoutes.get('/measurements/:audit_token', async (c) => {
  try {
    const { env } = c;
    const audit_token = c.req.param('audit_token');
