...
// ============================================================================
// GET /api/planning/feed/all.ics - Flux d'abonnement global (iCal Subscription)
// ============================================================================
planningRoutes.get('/feed/all.ics', async (c) => {
  try {
    const { DB } = c.env;

    // Récupérer toutes les interventions futures ou récentes (ex: -1 mois à +1 an)
    const query = `
      SELECT 
        i.*,
    ...
    const { results } = await DB.prepare(query).all<InterventionWithDetails>();
    ...
    const now = new Date();
    const nowStr = formatDateICS(now.toISOString());

    let icsBody = '';

    if (results && results.length > 0) {
      icsBody = results.map(i => {
        // Date début
        const startDate = new Date(i.intervention_date as string);
        if (startDate.getHours() === 0) startDate.setHours(8); // Défaut 8h
        
        // Date fin
        const endDate = new Date(startDate);
        endDate.setHours(endDate.getHours() + (i.duration_hours as number || 4));

        return [
          'BEGIN:VEVENT',
          `UID:intervention-${i.id}@diagpv.fr`,
          `DTSTAMP:${nowStr}`,
          `DTSTART:${formatDateICS(startDate.toISOString())}`,
          `DTEND:${formatDateICS(endDate.toISOString())}`,
          `SUMMARY:DiagPV: ${i.project_name || 'Projet'} (${i.intervention_type})`,
          `DESCRIPTION:Client: ${i.client_name || 'N/A'}\\nTech: ${i.technician_email || 'Non assigné'}\\nNotes: ${i.notes || ''}`,
          `LOCATION:${i.project_location || ''}`,
          'STATUS:CONFIRMED',
          'END:VEVENT'
        ].join('\r\n');
      }).join('\r\n');
    ...
    const icsContent = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//DiagPV//Planning Global//FR',
      'X-WR-CALNAME:DiagPV Planning',
      'CALSCALE:GREGORIAN',
      'METHOD:PUBLISH',
      icsBody,
      'END:VCALENDAR'
    ...
    return c.text(icsContent, 200, {
      ...
      'Content-Disposition': 'inline; filename="planning-diagpv.ics"',
      'Cache-Control': 'no-cache, no-store, must-revalidate'
    });

  } catch (error: any) {
    console.error('Erreur Feed ICS:', error);
    return c.text('Erreur génération Flux ICS', 500);
  }
});
...
planningRoutes.post('/interventions', async (c) => {
  try {
    ...
    const interventionId = result.meta.last_row_id;

    // ========================================================================
    // AUTO-CREATION AUDIT UNIFIÉ
    // ========================================================================
    try {
      // 1. Récupérer infos projet pour l'audit
      const projectInfo = await DB.prepare(`
        SELECT p.name, p.client_id, p.address_city, p.address_street, p.address_postal_code, 
               c.company_name 
        FROM projects p 
      ...
      if (projectInfo) {
        ...
        // Déterminer modules par défaut selon type intervention
        let modulesEnabled = ['VISUAL']; // Toujours visuel par défaut
        if (body.intervention_type === 'el_audit' || body.intervention_type === 'el') modulesEnabled.push('EL');
        if (body.intervention_type === 'iv_test' || body.intervention_type === 'iv') modulesEnabled.push('IV');
        if (body.intervention_type === 'thermography') modulesEnabled.push('THERMAL');
        if (body.intervention_type === 'isolation_test') modulesEnabled.push('ISOLATION');

        const location = [
          projectInfo.address_street, 
          projectInfo.address_postal_code, 
      ...
      console.error('⚠️ Erreur auto-création audit:', auditError);
      // On ne bloque pas la création de l'intervention si l'audit échoue
    }
...
// ============================================================================
// GET /api/planning/interventions/:id/ics - Export Outlook (.ics)
// ============================================================================
planningRoutes.get('/interventions/:id/ics', async (c) => {
  try {
    const { DB } = c.env;
    const id = parseInt(c.req.param('id'));

    // Récupérer intervention complète
    ...
    if (!intervention) {
      return c.text('Intervention introuvable', 404);
    }
    ...
    // Calcul date fin
    const startDate = new Date(intervention.intervention_date as string);
    // Ajuster l'heure si nécessaire (ici on suppose 08:00 par défaut si pas d'heure précise)
    if (startDate.getHours() === 0) startDate.setHours(8); // Début journée par défaut
    
    const endDate = new Date(startDate);
    endDate.setHours(endDate.getHours() + (intervention.duration_hours as number || 4));

    const now = new Date();
    
    // Construction contenu ICS
    const icsContent = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//DiagPV//Planning//FR',
      'CALSCALE:GREGORIAN',
      'METHOD:PUBLISH',
      'BEGIN:VEVENT',
      `UID:intervention-${intervention.id}@diagpv.fr`,
      `DTSTAMP:${formatDateICS(now.toISOString())}`,
      `DTSTART:${formatDateICS(startDate.toISOString())}`,
      `DTEND:${formatDateICS(endDate.toISOString())}`,
      `SUMMARY:Intervention DiagPV - ${intervention.project_name || 'Projet'}`,
      `DESCRIPTION:Client: ${intervention.client_name || 'N/A'}\\nType: ${intervention.intervention_type}\\nNotes: ${intervention.notes || ''}`,
      `LOCATION:${intervention.project_location || ''}`,
      'STATUS:CONFIRMED',
      'END:VEVENT',
      'END:VCALENDAR'
    ].join('\r\n');
    ...
    return c.text(icsContent, 200, {
      'Content-Type': 'text/calendar; charset=utf-8',
      'Content-Disposition': `attachment; filename="intervention-${id}.ics"`
    });

  } catch (error: any) {
    console.error('Erreur ICS:', error);
    return c.text('Erreur génération ICS', 500);
  }
});
