  ...
  return c.html(`
    ...
    <head>
        ...
        <style>
            ...
            /* Styles pour le canvas designer */
            #layoutCanvas {
                background: #f8f9fa;
                background-image: 
    ...
    <body class="bg-gray-50">
        ...
        <main id="contentDesigner" class="tab-content hidden p-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <div class="flex space-x-2">
                            <button onclick="clearLayout()" class="px-3 py-2 bg-red-500 hover:bg-red-600 text-white rounded-lg text-sm">
                                <i class="fas fa-trash mr-1"></i>Reset
                            </button>
                            <button onclick="saveLayout()" class="px-3 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg text-sm">
            ...
            <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
                ...
                <!-- Canvas scrollable -->
                <div class="border-2 border-gray-300 rounded-lg overflow-auto" style="height: 500px;">
                    <canvas 
                        id="layoutCanvas"
                        width="1200" 
                        height="800"
                        ...
                        onmousemove="handleCanvasHover(event)">
                    </canvas>
                </div>
        ...
        // NOUVEAU: Gestion des onglets et designer layout
        let layoutData = {
            modules: [],
            config: {
                moduleLength: 1960,
        ...
        // Initialisation du canvas designer
        ...
        function handleCanvasClick(event) {
            ...
            const moduleX = Math.floor(x / (layoutData.config.moduleWidth * 0.1 + layoutData.config.spacing));
            const moduleY = Math.floor(y / (layoutData.config.moduleLength * 0.1 + layoutData.config.spacing));
            
            const moduleId = generateModuleId(moduleX, moduleY);
            
            if (currentMode === 'add') {
                addModule(moduleX, moduleY, moduleId);
            } else {
                removeModule(moduleX, moduleY);
            }
        }
        
        // Génération ID module selon type de numérotation
        function generateModuleId(x, y) {
            ...
            switch (type) {
                ...
                case 'numeric':
                    return String(y * 20 + x + 1).padStart(3, '0');
                case 'rowcol':
                    return \`R\${y + 1}C\${x + 1}\`;
                default:
                    return \`\${x}-\${y}\`;
        ...
        // Ajouter un module
        function addModule(x, y, id) {
            const existingIndex = layoutData.modules.findIndex(m => m.x === x && m.y === y);
            
            if (existingIndex === -1) {
                layoutData.modules.push({
                    x, y, id,
                    hasDefect: false, // Pour corrélation future avec EL
                    timestamp: Date.now()
                });
        ...
        // Supprimer un module
        function removeModule(x, y) {
            layoutData.modules = layoutData.modules.filter(m => !(m.x === x && m.y === y));
            redrawCanvas();
            updateStats();
            saveLayoutToSystem();
        }
        ...
        function redrawCanvas() {
            ...
            const moduleW = layoutData.config.moduleWidth * 0.1; // Échelle 1:10
            const moduleH = layoutData.config.moduleLength * 0.1;
            const spacing = layoutData.config.spacing;
            
            // Dessiner les modules
            layoutData.modules.forEach(module => {
                const x = module.x * (moduleW + spacing);
                const y = module.y * (moduleH + spacing);
                
                // Couleur selon statut
                ctx.fillStyle = module.hasDefect ? '#EF4444' : '#3B82F6';
                ctx.strokeStyle = module.hasDefect ? '#DC2626' : '#1D4ED8';
                ctx.lineWidth = 1;
                ...
                ctx.fillText(module.id, x + moduleW/2, y + moduleH/2 + 3);
            });
        }
        ...
        function clearLayout() {
            if (confirm('Êtes-vous sûr de vouloir effacer tout le layout ?')) {
                layoutData.modules = [];
                redrawCanvas();
                updateStats();
                saveLayoutToSystem();
        ...
        function exportLayoutImage() {
            ...
            link.download = \`layout_\${new Date().toISOString().slice(0, 10)}.png\`;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ['moduleLength', 'moduleWidth', 'modulePower', 'moduleSpacing', 'numberingType'].forEach(id => {
