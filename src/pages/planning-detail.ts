...
<body class="min-h-screen">
        ...
        <div id="contentContainer" class="hidden space-y-6">
            ...
            <div class="flex items-center justify-between">
                ...
                <div class="flex space-x-3">
                    <button 
                        id="btnExportICS" 
                        class="px-6 py-3 border border-blue-600 text-blue-600 bg-white rounded-lg hover:bg-blue-50 font-medium"
    ...
    <script>
        ...
        // Charger audit unifié associé
        async function loadAssociatedAudit() {
            try {
                // Utiliser l'endpoint unifié
                const response = await fetch(\`/api/audits?intervention_id=\${interventionId}\`);
                const data = await response.json();
                ...
                if (data.success && data.audits && data.audits.length > 0) {
                    ...
                    // Parsing modules enabled
                    let modules = [];
                    try {
                        modules = JSON.parse(audit.modules_enabled || '[]');
                    } catch(e) {}
                    ...
                    document.getElementById('auditInfo').innerHTML = \`
                        ...
                        <div class="info-row">
                            ...
                            <div class="info-value">
                                \${modules.map(m => \`<span class="inline-block bg-blue-100 text-blue-800 text-xs px-2 py-1 rounded mr-1">\${m}</span>\`).join('')}
                            </div>
                        </div>
                        <div class="info-row">
                            <div class="info-label">
                        ...
                        <div class="p-6 bg-gray-50 flex space-x-3">
                            <a 
                                href="/audit/\${audit.audit_token}" 
                                class="inline-block flex-1 text-center px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium"
                            >
                                <i class="fas fa-tablet-alt mr-2"></i>
                                Accéder à l'Audit Unifié
                            </a>
                        </div>
                    \`;
                } else {
                    // Si pas d'audit, proposer création (Fallback)
                    document.getElementById('auditCard').classList.remove('hidden');
                    document.getElementById('auditInfo').innerHTML = \`
                        <div class="p-8 text-center">
                            ...
                            <p class="text-gray-600 mb-6">
                                L'audit n'a pas été créé automatiquement. Vous pouvez le générer maintenant.
                            </p>
                            <button 
                                id="btnCreateAudit"
                                class="px-8 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium text-lg shadow-md"
                            >
        ...
        // Créer audit unifié depuis intervention
        async function createAuditFromIntervention() {
            if (!currentIntervention) return;
            
            if (!confirm('Générer un audit unifié pour cette intervention ?')) return;

            try {
                // Utiliser create-multi-modules avec intervention_id
                const response = await fetch('/api/audits/create-multi-modules', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        intervention_id: parseInt(interventionId),
                        modules: ['VISUAL', 'EL', 'IV'] // Default set
                    })
                });
                ...
                alert('Audit généré avec succès !');
                loadAssociatedAudit();
        ...
        document.getElementById('btnExportICS').addEventListener('click', () => {
            window.location.href = \`/api/planning/interventions/\${interventionId}/ics\`;
        });
        ...
        function getAuditStatusBadge(status) {
            const badges = {
                ...
                'en_cours': '<span class="status-badge bg-yellow-100 text-yellow-800">En cours</span>'
            };
            return badges[status] || status;
        }
