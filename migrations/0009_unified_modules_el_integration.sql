...
-- SCRIPT MANUEL (à exécuter si besoin) :
/*
UPDATE pv_modules 
SET 
  el_defect_type = (
    SELECT em.defect_type 
    FROM el_modules em 
    WHERE em.module_identifier = pv_modules.module_identifier
      AND em.string_number = pv_modules.string_number
      AND em.position_in_string = pv_modules.position_in_string
    LIMIT 1
  ),
  el_severity_level = (
    SELECT em.severity_level 
    FROM el_modules em 
    WHERE em.module_identifier = pv_modules.module_identifier
      AND em.string_number = pv_modules.string_number
      AND em.position_in_string = pv_modules.position_in_string
